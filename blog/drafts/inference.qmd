...
# Please note the basis of this file is the NextGenDID.Rmd setup in HPOL8539-F2022_learnr project. 
...
make_data <- function(params, N = 500, start = 2005, stop = 2022,rho_t = 0.8) {
  ...
  # Year fixed effects (allowing for auto-correlation within goups across time)
  year <-
  ...
  df <-
    ...
    mutate(bin1 = bin[1],
           bin2 = bin[2]) %>% 
    select(unit,state, year, y_it = dep_var, w_it = treat,x_i, x_it, group, starts_with("rel_year"))
...
#df_ <- params6 %>% make_data(params=., start = 2005) %>% pluck("df") %>% prepare(params = params6)
...
# set.seed(23)
# ans <- params %>% make_data()  %>% pluck("df")  %>% prepare(params) %>% estimate() %>% discriminate()
...
#| message: false
df <- sim$df %>% 
  select(unit, state, year, y_it, d_it = w_it, x_i , x_it) %>% 
  group_by(unit) %>% 
  mutate(ever_treated = max(d_it))
df %>% head() %>% kable(digits = 3) 
```
