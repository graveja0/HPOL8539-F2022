...
set.seed(123456)
df_es <- params_es %>% make_data(N = 2000,start = 2005,stop = 2022)
df_es$truth %>% write_rds(here("ignore/eDID-case-study-data/ES-data_tr.rds"))
# df <-  df_es$df %>% select(y_it = dep_var,  year, state, w_it = treat, x_i, x_it, unit, contains("rel_year")) 

rel_names = grep("rel_year_",names(df_es$df),value=TRUE)
...
df_es_final <- 
  ...
  select(y_it = dep_var,  year, state, w_it = treat, x_i, x_it, unit,rel_year,rel_vars$rel_names,pre,post)   %>% 
  select(-rel_year)

df_es_final %>% write_rds(here("ignore/eDID-case-study-data/ES-data.rds"))

# feols(y_it ~ pre + rel_year_lag5 + rel_year_lag4 + rel_year_lag3 + rel_year_lag2 + rel_year_0 + rel_year_1  + rel_year_2 + rel_year_3 + rel_year_4 + rel_year_5 + post + factor(year) | unit, data = df_es_final) %>% summary()

# plot_data(df_es)
# Extended Two-Way Fixd Effects

params_cs =  list(
  desc = "Case Study Params",
  mu = list("2014" = 1,
            "2015" = 0.5,
            "2016" = 0),
  mu_t = list("2014" = -.05,
...
set.seed(123456)
df_cs <- params_cs %>% make_data(N = 10000,start = 2012,stop = 2018)
df_cs$truth %>% write_rds(here("ignore/eDID-case-study-data/eDID-data_tr.rds"))
df_cs <- df_cs$df %>% select(y_it = dep_var,  year, state, w_it = treat, x_i, x_it, unit)
...
df_final <- 
  ...
  left_join(d_star_cs,"unit") %>% 
  mutate_at(vars(starts_with("d_")),~ifelse(is.na(.),0,.)) %>% 
  dummy_cols("year",ignore_na=TRUE)


df_final %>% write_rds(here("ignore/eDID-case-study-data/eDID-data.rds"))
```
...
res_cs <- 
  with_progress({
  gen_est_disc(1:2000,params = params_cs, N = 2000,start = 2012,stop = 2018)
})

res_cs  %>% write_rds(here("blog/posts/results/extended-did-params-cs.rds"))
